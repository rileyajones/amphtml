...
function ingestUrl(url) {
  return new Promise(async (resolve, reject) => {
    debugLogger(green('Ingesting'), `: ${url}`);
    try {
      const start = Date.now();
...
/**
 * Retrieve the response from the cache if available, otherwise refetch.
 * @param {string} url
 * @return {Promise<string>}
 */
async function retrieveOrFetch(url) {
  debugLogger('getting url: ', url);
 ...
 * @param {string[]} output
 */
function debugLogger(...output) {
  log(...output);
  fs.appendFileSync('junit/test-results/test-cache.log', output.join(' '));
}
